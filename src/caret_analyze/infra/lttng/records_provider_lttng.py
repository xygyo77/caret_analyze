...
class RecordsProviderLttng(RuntimeDataProvider):
    ...
    def _subscribe_records(
        ...
        # --- ここからデバッグプリントを追加 ---
        print(f"DEBUG: Final columns from _subscribe_records(): {sub_records.columns}")
        print(f"DEBUG: Sample data (first 5 rows) from _subscribe_records():\n{sub_records.to_dataframe().head()}")
        # --- ここまでデバッグプリント ---
    ...
    def is_intra_process_communication(self, communication_value: CommunicationStructValue) -> bool:
        """
        Check if the communication is intra-process.
        ...
        -------
        bool
            True if intra-process communication, False otherwise.

        """
        # subscriptionまたはcallback情報が存在しない場合の早期リターン
        if communication_value.subscription is None or \
           communication_value.subscription.callback is None:
            logger.warning(f"Required subscription or callback information is None for topic: {communication_value.topic_name}")
            return False

        # サブスクライブ側のレコードを取得
        # ここで取得されるall_subscribe_recordsの'callback_object'カラムは、
        # LTTngトレースで記録されたサブスクリプションコールバックのオブジェクトID。
        # おそらくこれは 'callback_object_intra' に対応するもの。
        all_subscribe_records = self._subscribe_records(communication_value.subscription)

        # CommunicationStructValue.subscription.callback は SubscriptionCallbackStructValue です。
        # このコールバックに対応する「プロセス内通信用のオブジェクトID」を取得します。
        # get_subscription_callback_object_intra が直接そのIDを返します。
        try:
            target_intra_callback_object = self._helper.get_subscription_callback_object_intra(
                communication_value.subscription.callback
            )
        except Exception as e:
            logger.error(f"Error getting intra-process callback object from helper for topic {communication_value.topic_name}: {e}")
            return False

        # target_intra_callback_object が None の場合、プロセス内通信のオブジェクトIDが存在しない。
        if target_intra_callback_object is None:
            return False

        # all_subscribe_records の dataframe 内の 'callback_object' カラムに、
        # 取得したプロセス内通信用のオブジェクトIDが存在するかどうかをチェック
        # isin() にはリストを渡す必要があるので [target_intra_callback_object] とします。
        is_intra = (all_subscribe_records
                    .to_dataframe()[COLUMN_NAME.CALLBACK_OBJECT]
                    .isin([target_intra_callback_object])
                    .any())
        return is_intra
    ...
    def _compose_inter_proc_comm_records(
        ...
        RecordsInterface
            ...
            - [topic_name]/rmw_take_timestamp (Added)
        ...
        columns += [
            COLUMN_NAME.SOURCE_TIMESTAMP,
            COLUMN_NAME.RMW_TAKE_TIMESTAMP, # Add rmw_take_timestamp to ensure it's present
            COLUMN_NAME.CALLBACK_START_TIMESTAMP,
        ]
        ...
        # records オブジェクトを pandas.DataFrame に変換
        df_records = records.to_dataframe()

        print(f"\n--- DEBUG: Records generated by {self.__class__.__name__} before return (as DataFrame) ---")
        print(f"Columns: {df_records.columns.tolist()}")
        print(f"Shape: {df_records.shape}")

        # planning_validator に関連するカラムを探し、その行をダンプ
        planning_validator_cols = [col for col in df_records.columns if 'planning_validator' in col]

        if planning_validator_cols:
            print(f"Detected planning_validator related columns: {planning_validator_cols}")
            
            # これらのカラムを含む行をすべて表示 (NA値の確認も含む)
            relevant_data = df_records[planning_validator_cols]
            
            if not relevant_data.empty: # 関連するデータが存在するか確認
                if relevant_data.isna().any().any(): # planning_validator関連カラムのいずれかにNAがあるか
                    print("NA found in planning_validator related columns. Displaying relevant rows (full row data):")
                    # NAがある行を特定し、その行のすべてのカラムを表示
                    na_rows_indices = relevant_data.index[relevant_data.isna().any(axis=1)].tolist()
                    if na_rows_indices:
                        print(df_records.loc[na_rows_indices])
                    else:
                        print("No NA rows found despite .isna().any().any() being true. (This indicates an issue with NA detection logic or data state)")
                else:
                    print("No NA found in planning_validator related columns. Displaying some rows (head of relevant columns):")
                    print(relevant_data.head()) # NAがない場合は先頭数行を表示
            else:
                print("Relevant data for planning_validator columns is empty.")

        else:
            print("No planning_validator related columns found in generated records.")
...
class NodeRecordsUseLatestMessage:
    ...
    def to_records(self):
        ...
        if is_take_node:
            ...
            print(f'### [1] sub_records: {sub_records.to_dataframe()=}')
            ...
            print(f'### [2] sub_records: {sub_records.to_dataframe()=}')
        pub_records = self._provider.publish_records(self._node_path.publisher)
        print(f'### pub_records: {pub_records.to_dataframe()=}')
        ...
        if is_take_node:
            for column in sub_records.columns:
                if column.endswith(COLUMN_NAME.RMW_TAKE_TIMESTAMP):
                    print(f'!!!!!!!!! UseLatestMessage: set left_key to {column}')
        ...
        print(f'### pub_sub_records: {pub_sub_records.to_dataframe()=}')
        drop_columns = list(set(pub_sub_records.columns) - set(columns))
        pub_sub_records.drop_columns(drop_columns)
        pub_sub_records.reindex(columns)
        print(f'### FIX pub_sub_records: {pub_sub_records.to_dataframe()=}')
        return pub_sub_records
